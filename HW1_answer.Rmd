...
for (j in 1:n) {
  ...
  for (i in 1:8) {
    pred = predict(best.fit, prostate_train[-cv_dat_index, ], id = i) 
...
pred.bs.cv = predict(bestfit.bs.cv, prostate_test, id = 7) 
test_error.bs.cv = mean((prostate_test$lpsa - pred.bs.cv) ^ 2)

```

Based on CV results, the best subset size is 7, so we use the whole training data to build the model and then test on test data, we get the test.error = `r test_error`.

## (b) Best-subset linear regression with k chosen by BIC.

```{r}
bs.bic.frame = tibble(size = c(1:8),
                      BIC.value = ummary(best.fit)$bic)

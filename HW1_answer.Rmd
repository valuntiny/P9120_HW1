...
Based on CV results, the best subset size is 7, so we use the whole training data to build the model and then test on test data, we get the test.error = `r test_error.bs.cv`.
...
Based on BIC results, the best subset size is 2, so we use the whole training data to build the model and then test on test data, we get the test.error = `r test_error.bs.bic`.
...
Based on CV results, the best $\lambda$ = `r bestfit.lasso.cv$lambda.min`, so we use the whole training data to build the model and then test on test data, we get the test.error = `r test_error.lasso.cv`.
...
Based on BIC results, the best $\lambda$ = `r best.lambda`, so we use the whole training data to build the model and then test on test data, we get the test.error = `r test_error.lasso.bic`.
